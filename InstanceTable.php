...
class InstanceTable extends AbstractExternalModule
{
    ...
    protected function setTaggedFields()
    {
        ...
        foreach ($instrumentFields as $fieldName => $fieldDetails) {
            ...
            if (
                $fieldDetails['field_type'] === 'descriptive' &&
    ...
    public function getInstanceData($record, $event, $form, $filter, $includeFormStatus = true)
    {
        ...
        foreach ($repeatingFormFields as $repeatingFormFieldName => $repeatingFormFieldDetails) {
            // ignore descriptive text fields and fields tagged @FORMINSTANCETABLE_HIDE
            if (
                $repeatingFormFieldDetails['field_type'] !== 'descriptive' &&
        ...
        <script type="text/javascript">
            ...
            var <?php echo self::MODULE_VARNAME; ?> = (function(window, document, $, app_path_webroot, pid, simpleDialog, undefined) { // var MCRI_FormInstanceTable ...
                ...
                function refreshTableDialog() {
                    simpleDialog('Refresh the table contents and display any changes (instances added, updated or deleted).', 'Refresh Table?', null, 500, null, langNo, function() {
...
<?php
    ...
    protected function popupViewTweaks()
    {
        ...
        // JS
        $replace = array(
            "§DATA_ENTRY_190" => htmlentities($this->lang['data_entry_190'], ENT_QUOTES),
            "§DATA_ENTRY_234" => htmlentities($this->lang['data_entry_234'], ENT_QUOTES),
            "§DATA_ENTRY_239" => htmlentities($this->lang['data_entry_239'], ENT_QUOTES),
            "§DATA_ENTRY_275" => htmlentities($this->lang['data_entry_275'], ENT_QUOTES),
            "§DATA_ENTRY_276" => htmlentities($this->lang['data_entry_276'], ENT_QUOTES),
            "§DATA_ENTRY_432" => htmlentities($this->lang['data_entry_432'], ENT_QUOTES),
            "§ADD_NEW" => isset($_GET['extmod_instance_table_add_new']) ? "true" : "false",
            "§INSTANCE" => $_GET['instance'],
            "§REQ_MSG" => isset($_GET['__reqmsg']) ? "true" : "false",
        );

        $js = file_get_contents(__DIR__ . "/js/instance-table-popup.js");
