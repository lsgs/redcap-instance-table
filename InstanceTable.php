...
class InstanceTable extends AbstractExternalModule
{
    ...
    protected function pageTop() {
        // find any descriptive text fields tagged with @INSTANCETABLE=form_name
            ...
            if ($fieldDetails['field_type']==='descriptive' &&
                ...
                foreach ($includeVars as $idx => $fieldName) {
                    ...
                    if ($repeatingFormFields[$fieldName]['field_type']==='descriptive') {
                        ...
                        // ignore fields tagged @INSTANCETABLE_HIDE
                        $matches = array();
    ...
    public function getInstanceData($record, $event, $form, $fields, $filter, $includeFormStatus=true) {
        ...
        if (!empty($recordData[$record]['repeat_instances'][$event][$formKey])) {
            $allInstances = array_keys($recordData[$record]['repeat_instances'][$event][$formKey]);
            foreach ($recordData[$record]['repeat_instances'][$event][$formKey] as $instance => $instanceFieldData) {
                $thisInstanceValues = array();
                $thisInstanceValues[] = $this->makeInstanceNumDisplay($instance, $record, $event, $form, $instance, $allInstances);
    ...
    protected function makeOpenPopupAnchor($val, $record, $event, $form, $instance, $allInstances) {
        if ($this->isSurvey) {
            return $val;
        }
        return '<a title="Open instance" href="javascript:;" onclick="'.self::MODULE_VARNAME.'.editInstance(\''
            .$record.'\','.$event.',\''.$form.'\','.$instance.',\''.implode(',', $allInstances).'\');">'
            .$val.'</a>';
    }

    protected function makeInstanceNumDisplay($val, $record, $event, $form, $instance, $allInstances) {
        return $this->makeOpenPopupAnchor($val, $record, $event, $form, $instance, $allInstances);
    }

    protected function makeFormStatusDisplay($val, $record, $event, $form, $instance, $allInstances) {
        switch ($val) {
            case '2':
        ...
        return $this->makeOpenPopupAnchor($circle, $record, $event, $form, $instance, $allInstances);
    }
...
//                $cachedLabel = db_result(db_query("select label from redcap.redcap_web_service_cache where project_id=".db_escape($PROJECT_ID)." and service='".db_escape($service)."' and category='".db_escape($category)."' and `value`='".db_escape($val)."'"), 0);
      ...
      <script type="text/javascript">
        ...
        (function (window, document, $, app_path_webroot, pid, simpleDialog, undefined) { // var MCRI_FormInstanceTable ...
          ...
          return {
            ...
            editInstance: function (record, event, form, instance, instances) {
              var ref = (instances=='')?'':'&instances='+instances;
              instancePopup('View instance', record, event, form, instance + ref);
              return false;
            }
          }
    ...
    protected function popupViewTweaks() {
        ...
        if (isset($Proj->forms[$_GET['page']]['survey_id'])
            && $user_rights['forms'][$_GET['page']] == '3'
            && isset($_GET['editresp'])) {
            $delFormAlertMsg .= RCView::div(array('style'=>'margin-top:15px;color:#C00000;'),
                RCView::tt("data_entry_241"));
        }
        $delFormAlertMsg .= RCView::div(array('style'=>'margin-top:15px;color:#C00000;'),
            RCView::tt_i("data_entry_559", array($_GET['instance'])));
        $delFormAlertMsg .= RCView::div(array('style'=>'margin-top:15px;color:#C00000;font-weight:bold;'),
            RCView::tt("data_entry_190"));
      ...
      <style type="text/css">
        ...
        #submit-btn-saveexitrecord, #submit-btn-savenextrecord { display: none; }
      </style>
      <script type="text/javascript">
        ...
        $.saveButtonState = function(url, buttonstate) {
          var btnstate = $.urlGetParam("btnstate")
          if (btnstate == null) {
            url = url + '&btnstate='+ buttonstate;
          } else {
            url = $.urlParamReplace(url, "btnstate", buttonstate);
          }
          return url;
        }

        $.urlParamRemove = function (url, name) {
          var results = new RegExp('[\?&]' + name + '=([^&#]*)').exec(url);
          if (results !== null) {
            return url.replace(name + '=' + results[1], '');
          } else {
            return url;
          }
        }
        ...
        $(document).ready(function() {
          ...
          // this is for next instance
          var instancesParam = $.urlGetParam('instances');
          if (instancesParam !==null && instancesParam !== '') {
            var instances = instancesParam.split(",");
          } else {
            $('#submit-btn-savenextinstance').css("display", "none");
          }
          ...
          $('#submit-btn-savenextinstance')
            // Save and Next Instance -- go to next instance of same parent form
            // circles back to first in instance when it gets to last instance
            // this button is hidden if list of instances is not provided (when adding new instance)
            .attr('name', 'submit-btn-savecontinue')
            .removeAttr('onclick')
            .text(<?php echo "'" . $this->lang['data_entry_276'] . "'"?>)
            .click(function (event) {
              var instance = $.urlGetParam("instance");
              var nextIndex = (instances.indexOf(instance) + 1) % instances.length;
              var redirectUrl = $.urlParamReplace(window.location.href, "instance", instances[nextIndex]);

              redirectUrl = $.saveButtonState(redirectUrl, "savenextinstance");
              event.preventDefault();
              window.opener.refreshTables();
              dataEntrySubmit(this);
          ...
          $('#submit-btn-savecontinue')
            ...
            .removeAttr('onclick')
            .click(function (event) {
              redirectUrl = $.saveButtonState(redirectUrl, "savecontinue");
              event.preventDefault();
              window.opener.refreshTables();
              dataEntrySubmit(this);
              window.setTimeout($.redirectUrl, 60, redirectUrl);
            });
          /* highjacking existing savenextform button for custom functionality*/
          // 'Save & New Instance' replaces 'Save & Next Form' button
          $('#submit-btn-savenextform')
            ...
            .click(function (event) {
              ...
              redirectUrl = $.urlParamRemove(redirectUrl, "instances");
              redirectUrl = $.saveButtonState(redirectUrl, "savenextform");
              event.preventDefault();
              window.opener.refreshTables();
              dataEntrySubmit(this);
              window.setTimeout($.redirectUrl, 500, redirectUrl);
    ...
    public function redcap_every_page_before_render($project_id) {
        if (isset($_POST['extmod_closerec_home'])) {
            ...
            // retains the user's last button as the default
